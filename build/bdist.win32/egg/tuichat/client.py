...
class Client:
    ...
    def main(self,):
        logo_obj = src.tuichat.tuichat_utils.ui.Logo('client')
        logo = logo_obj.logo
        print(logo)
        ...
        connection_info_obj = src.tuichat.tuichat_utils.ui.ConnectionInfo(host, port)
            ...
            try:
                ...
                for element in data:
                    ...
                    if data_dict['type'] == 'server_closed':
                        ...
                        self.data_queue.append(f'{src.tuichat.tuichat_utils.data_handler.get_time()} {data_dict["sender_address"]} - {data_dict["message"]}')
            except timeout:
                return
    ...
    def send_data(self,):
        while not self.freeze:
            try:
                ...
                if message_input != "/r":
                    message = src.tuichat.tuichat_utils.data_handler.Client.serialize_client_data(message_input, self.uuid, 'message')
    ...
    def disconnect(self,):
        ...
        try:
            self.freeze = True
            message = src.tuichat.tuichat_utils.data_handler.Client.serialize_client_data('', self.uuid, 'disconnect')
            self.sock.sendall(bytes(message, encoding="utf-8"))
            self.sock.close()
        except Exception as ex:
            print(ex)
