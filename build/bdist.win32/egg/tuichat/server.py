...
class Server:
    ...
    def main(self,):
        ...
        logo_obj = src.tuichat.tuichat_utils.ui.Logo('server')
        logo = logo_obj.logo if self.enable_ui else logo_obj.raw_logo
        print(logo)

        license_obj = src.tuichat.tuichat_utils.ui.License()
        copyright = license_obj.license if self.enable_ui else license_obj.raw_license
        print(copyright)

        external_ip, start_time = self.run_server()

        server_infotable_obj = src.tuichat.tuichat_utils.ui.ServerInfotable(start_time, self.port, self.max_connections, external_ip, self.enable_log, self.enable_ui)
    ...
    def configure(self,):
        while True:
            ...
            if answer == 'y':
                try:
                    src.tuichat.tuichat_utils.data_handler.clear_screen()
                    max_connections_input = src.tuichat.tuichat_utils.data_handler.Server.configuration_input('max_connections')
                    port_input = src.tuichat.tuichat_utils.data_handler.Server.configuration_input('port')

                    enable_log_input = src.tuichat.tuichat_utils.data_handler.Server.configuration_input('enable_log')
                    enable_ui_input = src.tuichat.tuichat_utils.data_handler.Server.configuration_input('enable_ui')
            ...
            for resource in conns:
                if resource is self.sock:
                    if len(self.connections) < self.max_connections:
                        ...
                        print(f'{src.tuichat.tuichat_utils.data_handler.get_time()} {address[0]} connected!')
    ...
    def get_data(self, conn, address,):
        try:
            ...
            for element in data:
                ...
                data = f'{src.tuichat.tuichat_utils.data_handler.get_time()} {address} - {data_dict["message"]}'
                print(data)
            ...
            print(f'{src.tuichat.tuichat_utils.data_handler.get_time()} {address} disconnected!')
    ...
    def send_messages(self, data_dict, address, type,):
        if self.enable_log:
            message = f'{src.tuichat.tuichat_utils.data_handler.get_time()} {address} {data_dict["message"]}\n'
            self.save_log(message, 'a')

        message = src.tuichat.tuichat_utils.data_handler.Server.serialize_server_data(data_dict['message'], address, self.uuid, type)
        message_to_sender = src.tuichat.tuichat_utils.data_handler.Server.serialize_server_data(data_dict['message'], '[You]', self.uuid, type)
        for client in self.connections:
    ...
    def run_server(self,):
        ...
        start_time = src.tuichat.tuichat_utils.data_handler.get_time()
